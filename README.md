...
- **Fájlrendszer**: Perzisztens **XDisk** (XD-x32) fájlrendszer legfeljebb 16 fájl számára (fájlonként max 4 KB mérettel), amely automatikusan menti és betölti a fájlokat az IDE merevlemez szektoraiból.
...
#### Teljes fordítás (Merevlemez lemezkép létrehozása)
Az alábbi parancs lefordítja a bootloadert, a kernelt és az összes C modult, majd összefűzi őket egy `myos.img` nevű 1.44 MB-os virtuális merevlemezképpé:
```bash
make
```
...
#### Futtatás IDE merevlemezképként
```bash
make run
```
Ez a parancs elindítja a `qemu-system-i386` emulátort a lefordított `myos.img` merevlemezképpel (IDE-ként csatolva).
...
| `sync` | Manuálisan elmenti a memóriában lévő fájlokat az XDisk (XD-x32) lemezre. | `sync` |
...
- **Filesystem**: Persistent **XDisk** (XD-x32) filesystem supporting up to 16 files (with a maximum of 4 KB per file), automatically saving to and loading from IDE hard disk sectors.
...
#### Full compilation (Creating a Hard Disk image)
The following command compiles the bootloader, kernel, and all C modules, then merges them into a 1.44 MB virtual hard disk image named `myos.img`:
```bash
make
```
...
#### Running as an IDE Hard Disk Image
```bash
make run
```
This command starts the `qemu-system-i386` emulator with the compiled `myos.img` hard disk image (attached as IDE primary master).
...
| `sync` | Manually saves the files from memory to the XDisk (XD-x32) disk. | `sync` |
